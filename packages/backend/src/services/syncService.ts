...
import { TodoStatus, TodoPriority, SyncStatus } from '@prisma/client';

export const syncService = {
  async syncTodos(userId: string, syncPayload: SyncPayload) {
    ...
    const formattedServerTodos = serverTodos.map(todo => ({
      ...
      syncStatus: todo.syncStatus.toLowerCase() as 'synced' | 'pending' | 'conflict',
    }));
    ...
    // Perform updates
    const updatePromises = toUpdate.map(todo =>
      prisma.todo.update({
        where: { id: todo.id },
        data: {
          title: todo.title,
          ...
          priority: todo.priority.toUpperCase() as TodoPriority,
    ...
    // Perform creates
    const createPromises = toCreate.map(todo =>
      prisma.todo.create({
        data: {
          id: todo.id,
          title: todo.title,
  ...
  async resolveConflict(userId: string, todoId: string, resolution: 'client' | 'server') {
  ...
  async getLastSyncTimestamp(userId: string) {
